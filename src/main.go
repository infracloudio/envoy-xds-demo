...
func main() {
	...
	<-signal
	for {
		domains := domainsreader.ReadFile()
		nodeId := cache.GetStatusKeys()[0]
		c := createClusters(domains, nodeId)
		l := createListener(domains)
		s := createSecret()
		createSnapshot(c, l, s, nodeId)
...
func createSecret() []types.Resource {
	var secretName = "server_cert"
	log.Infof(">>>>>>>>>>>>>>>>>>> creating Secret " + secretName)
	priv, err := ioutil.ReadFile("certs/envoy-proxy-server.key")
	if err != nil {
		log.Fatal(err)
	}
	pub, err := ioutil.ReadFile("certs/envoy-proxy-server.crt")
	if err != nil {
		log.Fatal(err)
	}
			...
			Type: &auth.Secret_TlsCertificate{
				TlsCertificate: &auth.TlsCertificate{
					CertificateChain: &core.DataSource{
						Specifier: &core.DataSource_InlineBytes{InlineBytes: pub},
					},
					PrivateKey: &core.DataSource{
						Specifier: &core.DataSource_InlineBytes{InlineBytes: priv},
					},
				},
			},
		},
...
func createListener(domains [][]string) []types.Resource {
...
func createRoutes(domains [][]string) []*route.Route {
	var routes []*route.Route
	for _, domainEntry := range domains {
		var targetHost = domainEntry[0]
		var targetPrefix = domainEntry[2]
		var clusterName = "service_" + targetHost + targetPrefix
		route := route.Route{
...
func createClusters(domains [][]string, nodeId string) []types.Resource {
	var clusters []types.Resource
	for _, domainEntry := range domains {
		var targetHost = domainEntry[0]
		var targetPort, _ = strconv.ParseUint(domainEntry[1], 10, 32)
		var targetPrefix = domainEntry[2]
		var clusterName = "service_" + targetHost + targetPrefix
		log.Infof(">>>>>>>>>>>>>>>>>>> creating cluster, targetHost, nodeID %s,  %s, %s", clusterName, targetHost, nodeId)
		//uctx := &envoyapiv2auth.UpstreamTlsContext{}
